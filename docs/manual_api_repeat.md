...
class ImageProcessor:
    ...
    def detect_birds(self, image: Image.Image) -> Dict:
        try:
            ...
            roi_image, crop_offset = self.crop_to_roi(enhanced_image)
            ...
            scores = results["scores"].cpu()
            labels = results["labels"].cpu()
            keep_indices = self.non_max_suppression(boxes, scores, 0.5)
            boxes = boxes[keep_indices]
            scores = scores[keep_indices]
            labels = labels[keep_indices]
            bird_indices = []
...
__all__ = ['ImageProcessor', 'ModelManager']
...
class ImageProcessor:
    ...
    def crop_to_roi(self, image: Image.Image, padding: float = 0.15) -> Tuple[Image.Image, Tuple[int, int]]:
        width, height = image.size
        gray = image.convert("L")
        edges = gray.filter(ImageFilter.FIND_EDGES)
        edge_points = []
...
def detect_objects(image_data, return_type='json', model_version=None):
    ...
    for i in range(min(len(results["boxes"])), 10):
        box = results["boxes"][i].tolist()
        label = model.config.id2label[results["labels"][i].item()]
        score = results["scores"][i].item()
        detection_results["objects"].append({"class": label, "score": round(score, 4), "box": [round(c, 2) for c in box]})
    detection_results["count"] = len(detection_results["objects"])
...
<head>
    ...
    <script src="https://cdn.jsdelivr.net/npm/echarts@5.4.3/dist/echarts.min.js"></script>
    <style>
        body { margin: 0; background-color: #0f1621; color: #fff; font-family: Arial, sans-serif; overflow-x: hidden; }
        #province-map { width: 100%; height: 80vh; }
        ...
        .province-info { padding: 20px; background: rgba(10,30,60,0.8); margin: 20px; border-radius: 8px; box-shadow: 0 4px 20px rgba(0, 100, 255, 0.2); border: 1px solid rgba(0, 180, 255, 0.3); transition: all 0.3s ease; }
        .province-info:hover { transform: translateY(-3px); box-shadow: 0 8px 30px rgba(0, 120, 255, 0.3); }
        .province-info h2 { margin-top: 0; color: #00b4ff; border-bottom: 1px solid rgba(0, 180, 255, 0.3); padding-bottom: 10px; font-size: 1.5em; }
        .province-info p { color: #b3e0ff; line-height: 1.8; margin: 8px 0; }
        .loading-overlay { position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0, 0, 0, 0.7); display: flex; justify-content: center; align-items: center; z-index: 2000; }
        .loading-spinner { width: 50px; height: 50px; border: 3px solid rgba(0, 180, 255, 0.3); border-top: 3px solid #00b4ff; border-radius: 50%; animation: spin 1s linear infinite; }
        @keyframes spin { 0% { transform: rotate(0deg); } 100% { transform: rotate(360deg); } }
        .monitor-panel { background: rgba(10, 30, 60, 0.9); border: 2px solid #0a2dae; border-radius: 10px; padding: 20px; margin: 20px; box-shadow: 0 0 20px rgba(0, 100, 255, 0.3); position: relative; overflow: hidden; }
        .monitor-panel::before { content: ''; position: absolute; top: -50%; left: -50%; width: 200%; height: 200%; background: linear-gradient(45deg, transparent, rgba(0, 180, 255, 0.1), transparent); animation: shine 3s infinite; }
        @keyframes shine { 0% { transform: translate(-50%, -50%) rotate(0deg); } 100% { transform: translate(-50%, -50%) rotate(360deg); } }
        .monitor-panel h3 { color: #00ff88; margin-top: 0; font-size: 1.2em; border-bottom: 1px solid rgba(0, 255, 136, 0.3); padding-bottom: 8px; }
        .data-row { display: flex; justify-content: space-between; padding: 8px 0; border-bottom: 1px solid rgba(0, 180, 255, 0.1); }
        .data-label { color: #b3e0ff; }
        .data-value { color: #00ff88; font-weight: bold; }
        .status-indicator { display: inline-block; width: 10px; height: 10px; border-radius: 50%; background: #00ff88; box-shadow: 0 0 10px #00ff88; margin-right: 8px; animation: pulse 1.5s ease-in-out infinite; }
        @keyframes pulse { 0%, 100% { opacity: 1; transform: scale(1); } 50% { opacity: 0.7; transform: scale(1.2); } }
        .chart-container { background: rgba(0, 40, 80, 0.5); border: 1px solid rgba(0, 180, 255, 0.3); border-radius: 8px; padding: 15px; margin-top: 15px; }
        .chart-title { color: #00b4ff; font-size: 1em; margin-bottom: 10px; }
        .tech-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 15px; margin-top: 15px; }
        .tech-item { background: rgba(0, 60, 120, 0.3); padding: 15px; border-radius: 6px; text-align: center; border: 1px solid rgba(0, 180, 255, 0.2); transition: all 0.3s ease; }
        .tech-item:hover { background: rgba(0, 80, 160, 0.5); transform: translateY(-3px); box-shadow: 0 5px 15px rgba(0, 120, 255, 0.3); }
        .tech-icon { font-size: 2em; margin-bottom: 8px; }
        .tech-name { color: #00b4ff; font-size: 0.9em; }
        .tech-status { color: #00ff88; font-size: 0.8em; margin-top: 5px; }
        .map-legend { position: absolute; bottom: 30px; right: 30px; background: rgba(10, 30, 60, 0.9); padding: 15px; border-radius: 8px; border: 1px solid rgba(0, 180, 255, 0.3); }
        .legend-item { display: flex; align-items: center; margin: 5px 0; color: #b3e0ff; font-size: 0.85em; }
        .legend-color { width: 20px; height: 10px; margin-right: 8px; border-radius: 2px; }
        .info-card { background: linear-gradient(135deg, rgba(10, 40, 80, 0.8), rgba(20, 60, 100, 0.8)); border-radius: 10px; padding: 20px; margin-top: 15px; border: 1px solid rgba(0, 180, 255, 0.3); }
        .info-card h4 { color: #00b4ff; margin: 0 0 10px 0; font-size: 1.1em; }
        .info-card p { color: #b3e0ff; margin: 5px 0; font-size: 0.9em; line-height: 1.6; }
        .highlight-text { color: #00ff88; font-weight: bold; text-shadow: 0 0 10px rgba(0, 255, 136, 0.5); }
        .data-update { font-size: 0.8em; color: #666; text-align: right; margin-top: 10px; }
    </style>
</head>
<body>
    ...
    <div id="province-map"></div>
    <div class="province-info">
        <h2><span class="status-indicator"></span>山西省信息</h2>
        <p>面积：156800平方公里</p>
        <p>人口：3481万人</p>
        <p>GDP：22590亿元</p>
        <div class="monitor-panel">
            <h3>监测面板</h3>
            <div class="data-row"><span class="data-label">监测站点数量</span><span class="data-value">128</span></div>
            <div class="data-row"><span class="data-label">在线设备</span><span class="data-value">115</span></div>
            <div class="data-row"><span class="data-label">今日数据量</span><span class="data-value">2.3GB</span></div>
            <div class="data-row"><span class="data-label">异常告警</span><span class="data-value highlight-text">0</span></div>
        </div>
        <div class="tech-grid">
            <div class="tech-item"><div class="tech-icon">🛰️</div><div class="tech-name">卫星遥感</div><div class="tech-status">● 在线</div></div>
            <div class="tech-item"><div class="tech-icon">📡</div><div class="tech-name">无人机巡检</div><div class="tech-status">● 在线</div></div>
            <div class="tech-item"><div class="tech-icon">📷</div><div class="tech-name">视频监控</div><div class="tech-status">● 在线</div></div>
        </div>
        <div class="chart-container">
            <div class="chart-title">实时数据流监控</div>
            <div id="data-flow-chart" style="height: 150px;"></div>
        </div>
        <div class="info-card">
            <h4>科技特点</h4>
            <p>采用<span class="highlight-text">空天地一体化</span>监测架构，整合卫星遥感、无人机巡检、地面传感器等多源数据。</p>
            <p>支持<span class="highlight-text">实时传输</span>与<span class="highlight-text">智能分析</span>，实现生态环境全方位监测。</p>
        </div>
        <div class="data-update">数据更新时间: <span id="update-time"></span></div>
    </div>
    <script>
        const mapChart = echarts.init(document.getElementById('province-map'));
        const provinceName = '山西省';
        const adcode = '140000';
        ...
        function initDataFlowChart() {
            const chartEl = document.getElementById('data-flow-chart');
            if (!chartEl) return;
            const flowChart = echarts.init(chartEl);
            const data = [];
            const now = new Date();
            for (let i = 0; i < 20; i++) {
                data.push({ value: [now.getTime() - (20 - i) * 60000, Math.random() * 100] });
            }
            flowChart.setOption({
                backgroundColor: 'transparent',
                tooltip: { trigger: 'axis' },
                xAxis: { type: 'time', splitLine: { show: false }, axisLabel: { color: '#b3e0ff' } },
                yAxis: { type: 'value', splitLine: { lineStyle: { color: 'rgba(0, 180, 255, 0.2)' } }, axisLabel: { color: '#b3e0ff' } },
                series: [{ type: 'line', smooth: true, data: data, lineStyle: { color: '#00b4ff' }, areaStyle: { color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [{ offset: 0, color: 'rgba(0, 180, 255, 0.5)' }, { offset: 1, color: 'rgba(0, 180, 255, 0)' }]) } }]
            });
        }
        ...
        function loadProvinceData() {
            ...
            $.ajax({
                url: mapUrl, type: 'GET', dataType: 'json',
                success: function(geoJson) {
                    hideLoading();
                    echarts.registerMap(provinceName, geoJson);
                    mapChart.setOption({
                        backgroundColor: '#0f1621',
                        title: { text: provinceName + '科技地图', left: 'center', textStyle: { color: '#fff', fontSize: 24 } },
                        tooltip: { trigger: 'item', backgroundColor: 'rgba(10, 40, 80, 0.9)', borderColor: '#00b4ff', textStyle: { color: '#fff' } },
                        geo: { map: provinceName, roam: true, zoom: 1.2, itemStyle: { areaColor: '#1a2b5a', borderColor: '#0a2dae', borderWidth: 2 }, emphasis: { itemStyle: { areaColor: '#2a91d8' } } }
                    });
                },
        ...
        $(document).ready(function() {
            ...
            initDataFlowChart();
            setInterval(initDataFlowChart, 60000);
            $(window).resize(function() { mapChart.resize(); });
        });
    </script>
</body>
</html>
    ...
    def __post_init__(self):
        if self.province_pages is None:
            self.province_pages = {
                ...
                "山西省": {"adcode": "140000", "area": "156800", "population": "3481", "gdp": "22590"},
