# 文件名: /opt/smart_monitor/config.py
================================================================================
"""
项目配置文件
...
# 文件名: /opt/smart_monitor/Backend/database.py
================================================================================
import sqlite3
...
# 文件名: /opt/smart_monitor/Backend/model_manager.py
================================================================================
"""
模型管理模块
...
# 文件名: /opt/smart_monitor/Backend/wrf_model_adapter.py
================================================================================
"""
WRF模式气象数据适配器模块
...
class WRFVariable:
    ...
    def min(self, axis: Optional[Union[int, Tuple[int, ...]]] = None) -> np.ndarray:
        """计算最小值"""
        return np.nanmin(self.data, axis=min)
...
class SurfaceVariable:
    ...
    sensible_heat_flux: WRFVariable = None  # 感热通量 (W/m2)
    latent_heat_flux: WRFVariable = None    # 潜热通量 (W/m2)
...
# 文件名: /opt/smart_monitor/Backend/api.py
================================================================================
# -*- coding: utf-8 -*-
"""
空天地一体化智能监测平台
...
# 文件名: /opt/smart_monitor/Backend/simulation_data.py
================================================================================
"""
热岛温度模拟数据生成模块
用于生成模拟的城市热岛温度矩阵数据
...
@dataclass
class UrbanHeatIslandConfig:
    """热岛模拟配置"""
    grid_resolution: float = 0.01  # 网格分辨率（度）
    urban_center_lon: float = 113.8  # 城市中心经度
    urban_center_lat: float = 34.8   # 城市中心纬度
    urban_radius: float = 0.3         # 城市区域半径（度）
    base_temperature: float = 298.0   # 基础温度（K）
    heat_island_intensity: float = 4.0  # 热岛强度（K）
    diurnal_amplitude: float = 8.0    # 日较差（K）
    time_step_hours: int = 1          # 时间步长（小时）
    simulation_days: int = 7           # 模拟天数


class UrbanHeatIslandSimulator:
    """城市热岛模拟器"""
    
    def __init__(self, config: Optional[UrbanHeatIslandConfig] = None):
        """
        初始化热岛模拟器
        ...
        Args:
            config: 模拟配置
        """
        self.config = config or UrbanHeatIslandConfig()
        self.grid_size = (50, 60)  # 默认网格大小
    ...
    def _create_coordinate_grid(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        Returns:
            Tuple[lon_grid, lat_grid]: 经度网格和纬度网格
        """
        grid_rows, grid_cols = self.grid_size
        
        # 计算网格范围
        lon_range = np.linspace(
            self.config.urban_center_lon - 0.5,
            self.config.urban_center_lon + 0.5,
            grid_cols
        )
        lat_range = np.linspace(
            self.config.urban_center_lat - 0.4,
            self.config.urban_center_lat + 0.4,
            grid_rows
        )
    ...
    def _create_urban_mask(self, lon_grid: np.ndarray, lat_grid: np.ndarray) -> np.ndarray:
        """
        创建城市区域掩码
        ...
        # 计算每个网格点到城市中心的距离
        distance = np.sqrt(
            (lon_grid - self.config.urban_center_lon)**2 +
            (lat_grid - self.config.urban_center_lat)**2
        )
        
        # 创建城市区域掩码
        urban_mask = distance < self.config.urban_radius
        
        return urban_mask
    
    def _create_lcz_mask(self, lon_grid: np.ndarray, lat_grid: np.ndarray) -> Dict[str, np.ndarray]:
        """
        创建LCZ（局地气候区）分类掩码
        ...
        # LCZ 1: 紧凑高层建筑（城市核心区）
        lcz_masks['LCZ1'] = (distance < 0.1)
        
        # LCZ 2: 紧凑中层建筑
        lcz_masks['LCZ2'] = (distance >= 0.1) & (distance < 0.15)
        
        # LCZ 3: 紧凑低层建筑
        lcz_masks['LCZ3'] = (distance >= 0.15) & (distance < 0.2)
        
        # LCZ 4: 开放高层建筑
        lcz_masks['LCZ4'] = (distance >= 0.2) & (distance < 0.25)
        
        # LCZ 5: 开放中层建筑
        lcz_masks['LCZ5'] = (distance >= 0.25) & (distance < 0.3)
        
        # LCZ 6: 开放低层建筑
        lcz_masks['LCZ6'] = (distance >= 0.3) & (distance < 0.35)
        
        # LCZ A: 茂密植被（公园绿地）
        center_y, center_x = np.shape(lon_grid)[0] // 2, np.shape(lon_grid)[1] // 2
        park_distance = np.sqrt(
            (np.arange(np.shape(lon_grid)[0])[:, np.newaxis] - center_y)**2 +
            (np.arange(np.shape(lon_grid)[1]) - center_x)**2
        )
        lcz_masks['LCZA'] = (park_distance < 8)
        
        # LCZ B: 稀疏植被
        lcz_masks['LCZB'] = ~np.any([lcz_masks[k] for k in lcz_masks.keys()], axis=0)
        
        return lcz_masks
    
    def _calculate_diurnal_temperature(self, hour: int) -> float:
        """
        计算某小时的温度日变化
        ...
        """
        # 日变化: 峰值在14:00，最低在05:00
        diurnal_offset = self.config.diurnal_amplitude * np.sin(2 * np.pi * (hour - 9) / 24)
        return diurnal_offset
    
    def _get_lcz_temperature_offset(self, lcz_type: str) -> float:
        """
        获取LCZ类型对应的温度偏移量
        ...
        return lcz_offsets.get(lcz_type, 0.0)
    
    def generate_static_temperature_field(self) -> np.ndarray:
        """
        生成静态温度场（不考虑时间变化）
        ...
        Returns:
            np.ndarray: 温度矩阵
        """
        logger.info("正在生成静态温度场...")
        
        # 创建坐标网格
        lon_grid, lat_grid = self._create_coordinate_grid()
        
        # 创建LCZ分类掩码
        lcz_masks = self._create_lcz_mask(lon_grid, lat_grid)
        
        # 初始化温度矩阵
        temperature = np.full(self.grid_size, self.config.base_temperature)
        
        # 根据LCZ类型填充温度
        for lcz_type, mask in lcz_masks.items():
            if np.any(mask):
                temperature[mask] += self._get_lcz_temperature_offset(lcz_type)
        
        # 添加随机噪声
        noise = np.random.normal(0, 0.5, self.grid_size)
        temperature += noise
        
        self.temperature_matrix = temperature
        
        logger.info(f"静态温度场生成完成，范围: {temperature.min():.2f}K - {temperature.max():.2f}K")
        
        return temperature
    
    def generate_time_series_temperature(self) -> pd.DataFrame:
        """
        生成时间序列温度数据
        
        Returns:
            pd.DataFrame: 时间序列数据框
        """
        logger.info("正在生成时间序列温度数据...")
        
        # 生成时间索引
        total_hours = self.config.simulation_days * 24
        start_time = datetime(2024, 7, 15, 0, 0, 0)
        times = [start_time + timedelta(hours=i) for i in range(total_hours)]
        ...
        # 生成各LCZ区域的时间序列
        time_series_data = {'datetime': times}
        ...
        for lcz_type, mask in lcz_masks.items():
            if np.any(mask):
                ...
                for hour in range(total_hours):
                    ...
                    diurnal_offset = self._calculate_diurnal_temperature(hour % 24)
                    lcz_offset = self._get_lcz_temperature_offset(lcz_type)
                    
                    temp = base_temp + diurnal_offset + lcz_offset
                    lcz_temperatures.append(temp)
                
                time_series_data[lcz_type] = lcz_temperatures
        ...
        logger.info(f"时间序列数据生成完成，共 {len(df)} 个时间点")
        
        return df
    
    def generate_full_spatiotemporal_data(self) -> np.ndarray:
        """
        生成完整的时空温度数据
        
        Returns:
            np.ndarray: 4D数组 (时间, 行, 列)
        """
        logger.info("正在生成完整的时空温度数据...")
        ...
        # 初始化4D数组
        spatiotemporal_data = np.zeros((total_hours, *self.grid_size))
        ...
        for hour in range(total_hours):
            ...
            # 初始化当前时间点的温度场
            temp_field = np.full(self.grid_size, self.config.base_temperature + diurnal_offset)
            
            # 应用LCZ偏移
            for lcz_type, mask in lcz_masks.items():
                if np.any(mask):
                    lcz_offset = self._get_lcz_temperature_offset(lcz_type)
                    temp_field[mask] += lcz_offset
            
            # 添加随机噪声
            noise = np.random.normal(0, 0.3, self.grid_size)
            temp_field += noise
            
            spatiotemporal_data[hour] = temp_field
        
        logger.info(f"时空数据生成完成，形状: {spatiotemporal_data.shape}")
    ...
    def calculate_heat_island_intensity(
        ...
        if self.time_series is None:
            self.generate_time_series_temperature()
        
        # 计算城区和农村的平均温度
        urban_avg_temp = self.time_series.mean(axis=1)
        
        # 创建农村区域的模拟数据
        rural_base = self.config.base_temperature - 1.0
        rural_temperatures = []
        
        for hour in range(len(self.time_series)):
            diurnal_offset = self._calculate_diurnal_temperature(hour % 24)
            temp = rural_base + diurnal_offset + np.random.normal(0, 0.2)
            rural_temperatures.append(temp)
        
        # 计算热岛强度
        heat_intensity = pd.DataFrame({
            'datetime': self.time_series.index,
            'urban_temperature': urban_avg_temp.values,
            'rural_temperature': rural_temperatures,
            'heat_intensity': [u - r for u, r in zip(urban_avg_temp.values, rural_temperatures)]
        })
        ...
        try:
            if format == 'csv':
                if self.time_series is not None:
                    self.time_series.to_csv(filepath)
                else:
                    logger.warning("无时间序列数据可导出")
                    return False
            elif format == 'npy':
                if self.temperature_matrix is not None:
                    np.save(filepath, self.temperature_matrix)
                else:
                    logger.warning("无温度矩阵可导出")
                    return False
            elif format == 'json':
                export_dict = {
                    'base_temperature': self.config.base_temperature,
                    'heat_island_intensity': self.config.heat_island_intensity,
                    'grid_size': self.grid_size,
                    'temperature_range': {
                        'min': float(self.temperature_matrix.min()) if self.temperature_matrix is not None else None,
                        'max': float(self.temperature_matrix.max()) if self.temperature_matrix is not None else None,
                        'mean': float(self.temperature_matrix.mean()) if self.temperature_matrix is not None else None
                    }
                }
                with open(filepath, 'w', encoding='utf-8') as f:
                    json.dump(export_dict, f, ensure_ascii=False, indent=2)
    ...
    def get_statistics(self) -> Dict:
        ...
        if self.temperature_matrix is None:
            self.generate_static_temperature_field()
        
        return {
            'grid_size': self.grid_size,
            'temperature_min': float(self.temperature_matrix.min()),
            'temperature_max': float(self.temperature_matrix.max()),
            'temperature_mean': float(self.temperature_matrix.mean()),
            'temperature_std': float(self.temperature_matrix.std()),
            'base_temperature': self.config.base_temperature,
            'heat_island_intensity': self.config.heat_island_intensity,
...
def main():
    ...
    # 创建模拟器实例
    simulator = UrbanHeatIslandSimulator()
    
    # 生成静态温度场
    temp_field = simulator.generate_static_temperature_field()
    print(f"静态温度场统计: {simulator.get_statistics()}")
    
    # 生成时间序列数据
    time_series = simulator.generate_time_series_temperature()
    print(f"\n时间序列数据 (前5行):\n{time_series.head()}")
    
    # 生成时空数据
    spatiotemporal = simulator.generate_full_spatiotemporal_data()
    print(f"\n时空数据形状: {spatiotemporal.shape}")
    
    # 导出数据
    simulator.export_temperature_data('temperature_field.csv', format='csv')
    simulator.export_temperature_data('temperature_stats.json', format='json')
