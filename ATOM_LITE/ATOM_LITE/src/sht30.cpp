...
static bool read_raw(float &temp, float &humid) {
    ...
    delay(30);
    int n = Wire.requestFrom((int)SHT30_ADDR, 6);
    if (n < 6) return false;
    uint8_t d[6];
...
bool sht30_init(int sda, int scl) {
    static const int pins[][2] = I2C_SCAN_PINS;
    _found = false;
    for (int t = 0; t < 3 && !_found; t++) {
        int a = pins[t][0], b = pins[t][1];
        Wire.begin(a, b, I2C_FREQ); delay(10);
        Wire.beginTransmission(SHT30_ADDR);
        if (Wire.endTransmission(true) == 0) {
            _found = true; do_reset();
            float dt, dh;
            for (int r = 0; r < 3; r++) { if (read_raw(dt, dh)) break; delay(50); }
        }
    }
    return _found;
}
